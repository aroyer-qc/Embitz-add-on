...
void RegisterISR(uint8_t VIC_Number, uint8_t Priority, uint32_t Address)
{
    ...
    pVectorAddress   = (uint32_t*)((uint32_t)VIC_BASE_ADDR + 100 + ((uint32_t)VIC_Number * 4));     /* Set the vector address.      */
    *pVectorAddress  = Address;
    pVectorPriority  = (uint32_t*)(VIC_BASE_ADDR + 200 + ((uint32_t)VIC_Number * 4));               /* Set the vector priority.     */
